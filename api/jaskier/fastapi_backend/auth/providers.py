...
from auth.util import create_state_csrf_token
...
class SpotifyAuthProvider(AuthProvider):
	...
	@staticmethod
	async def meets_condition(auth_provider):
		return auth_provider == config.SPOTIFY

	async def get_user(self, auth_token: ExternalAuthToken) -> ExternalUser:
		# Request access_token from Spotify
		token_url, headers, body = self.auth_client.prepare_token_request(
		...
		# Request user's information from Google
		uri, headers, body = self.auth_client.add_token(config.SPOTIFY_ME_URL)
		userinfo_response = requests.get(uri, headers=headers, data=body)
	...
	async def get_request_uri(self) -> str:
		authorization_endpoint = config.SPOTIFY_AUTH_URL
